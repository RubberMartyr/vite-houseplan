    ...
    for (let tri = 0; tri < triangleCount; tri += 1) {
      ...
      const onLeftSegment =
        ...
        LEFT_FACADE_SEGMENTS.some((segment) => {
          const outerX = segment.x;
          const innerX = segment.x + exteriorThickness; // inward = +X for negative-x facade
          const onOuterX =
 ...
 * Covers L_A / L_B / L_C with ground-floor window/door holes.
 */
export function makeLeftSegmentPanels(): FacadePanel[] {
  ...
  return LEFT_FACADE_SEGMENTS.map((segment) => {
    ...
    rightSideWindowSpecs.forEach((spec) => {
      ...
      const path = new Path();
      path.moveTo(zMin - panelCenterZ, spec.groundY0 - wallHeight / 2);
      path.lineTo(zMax - panelCenterZ, spec.groundY0 - wallHeight / 2);
      path.lineTo(zMax - panelCenterZ, spec.groundY1 - wallHeight / 2);
      path.lineTo(zMin - panelCenterZ, spec.groundY1 - wallHeight / 2);
      path.closePath();
      shape.holes.push(path);
    });

    const panelGeometry = new ShapeGeometry(shape);
    panelGeometry.rotateY(Math.PI / 2); // normal → −X (outward for negative-x facade)
    panelGeometry.computeVertexNormals();
