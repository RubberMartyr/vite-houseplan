...
function createFrontPortalDoorParts(
  ...
  const revealThickness = Math.max(surroundBand * 0.45, 0.045);
  const revealDepth = Math.max(wallThickness * 0.9, 0.26);
  const stoneCenterZ = stoneDepth / 2;
  const revealCenterZ = revealDepth / 2 + FRONT_PORTAL_STONE_PROJECTION * 0.25;
  ...
  const woodReveal = 0.02;
  const woodCenterZ = Math.max(woodDepth / 2 + woodReveal, stoneDepth * 0.24);
  ...
  return [
    ...
    {
      key: 'stone-return-left',
      material: 'stone',
      debugIgnore: true,
      size: [revealThickness, openingHeight, revealDepth],
      position: [-openingWidth / 2 + revealThickness / 2, 0, revealCenterZ],
    },
    {
      key: 'stone-return-right',
      material: 'stone',
      debugIgnore: true,
      size: [revealThickness, openingHeight, revealDepth],
      position: [openingWidth / 2 - revealThickness / 2, 0, revealCenterZ],
    ...
    {
      key: 'stone-return-top',
      material: 'stone',
      debugIgnore: true,
      size: [openingWidth, revealThickness, revealDepth],
      position: [0, openingHeight / 2 - revealThickness / 2, revealCenterZ],
    },
    {
      key: 'wood-leaf',
      material: 'wood',
...
export function resolveOpeningRenderParts(
  ...
  if (style?.variant === 'frontPortalDoor') {
    return {
      ...
      glassInset,
      glassThickness,
      originOffsetZ: wallThickness + FRONT_PORTAL_STONE_PROJECTION,
